...
class TestPage < Elementis::TestCase
  ...
  should "load page with non root path" do
    @app.account_page.load
    assert_equal "/products-page/your-account/", page.current_path
  end

  should "raise exception when page loaded with no url set" do
    @page.instance_eval { undef :url }
    assert_raises(PageUrlNotSetError) { @page.load }
  end
